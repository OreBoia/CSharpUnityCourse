...
app.get('/api/tasks/:id', async (req, res) => {
  try {
    const [tasks] = await db.execute('SELECT * FROM tasks WHERE id = ?', [req.params.id]);
    if (tasks.length === 0) {
      return res.status(404).json({ error: 'Task non trovata' });
    }
    res.json(tasks[0]);
  } catch (error) {
    res.status(500).json({ error: 'Errore nel recupero della task', details: error.message });
  }
});
...
app.put('/api/tasks/:id', async (req, res) => {
  try 
  {
    ...
    res.status(500).json({ error: 'Errore nell\'aggiornamento della task', details: error.message });
  }
});
